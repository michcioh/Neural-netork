...
{
    public partial class WindowDataForNeuralNetworks : Form
    {
        ...
        public WindowDataForNeuralNetworks()
        {
            InitializeComponent();
        }
        ...
        private void BSave_Click(object sender, EventArgs e)
        {
            ...
            for (int r = 0; r < dgvExample.Rows.Count; r++)
            {
                for (int c = 0; c < dgvExample.Rows.Count; c++)
                {
                    ...
                    row += dgvExample.Rows[r].Cells[c].Style.BackColor == Color.PaleGreen ? 1 : 0;
                }
            }
            ...
            for (int r = 0; r < dgvResult.Rows.Count; r++)
            {
                ...
                {
                    if (row.Length > 0) row += ";";
                    row += dgvResult.Rows[r].Cells[c].Style.BackColor == Color.PaleGreen ? 1 : 0;
                }
            }
            ...
            if (actualNetwork != null)
            {
                ...
                if (errMsg == null)
                {
                    ...
                    for (int r = 0; r < dgvResult.Rows.Count; r++)
                    {
                        ...
                        {
                            Console.WriteLine(
                                String.Format("Output no. {0} = {1, 6:0.000}",
                                idx,
                                result[idx])
                            );
                    ...
                    for (int r = 0; r < dgvResult.Rows.Count; r++)
                    {
                        for (int c = 0; c < dgvResult.Columns.Count; c++)
                        {
                            double percentage = ((int)(result[idx++] / sum * 100)); // get value in format x,xxxx
        ...
        {
            int rgbParam = (int)(100 - percentage) +100;
        ...
        private void LoadNetwork()
        {
            if (ofdOpen.ShowDialog(this) == DialogResult.OK)
            {
                ...
                try
                {
                    actualNetwork = NeuralNetwork.LoadNetworkFromFile(fiOpenFile.FullName); //Tools.DeSerializeObject<NeuralNetwork>(fiOpenFile.FullName);
                }
                catch (Exception ex)
                {
        ...
        private void nudLearningDataLineNo_ValueChanged(object sender, EventArgs e)
        {
            ...
            for (int r = 0; r < dgvExample.Rows.Count; r++)
            {
                for (int c = 0; c < dgvExample.Columns.Count; c++)
                {
                    ...
                    dgvExample.Rows[r].Cells[c].Value = value;
                }
            }
