...
namespace NeuralNet
{
    ...
    public class Neuron
    {
        ...
        public Neuron (int inputsCount, int layerNo, bool isBias, NeuralNetwork.ActivationFunction activationFunction, 
            double minWeight, double maxWeight, double biasValue = -1.0)
        {
            IsBias = isBias;
            BiasValue = biasValue;
            LayerNo = layerNo;
        ...
        public void ResetNeuron()
        {
            ...
            for (int inputNo = 0; inputNo < inputsCnt; inputNo++)
            {
                if (LayerNo == 0)
                {
                    Inputs.Add(new Input() { Value = 0.0, Weight = 0.0, ConnectedWithPreviousLayerNeuronNo = -1 });
                    PreviousInputs.Add(new Input() { Value = 0.0, Weight = 0.0, ConnectedWithPreviousLayerNeuronNo = -1 });
                }
                else
                {
                    double weight = new RandomWeight(MinWeight, MaxWeight).GenerateRandomWeight();
